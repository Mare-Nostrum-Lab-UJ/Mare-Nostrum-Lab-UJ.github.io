...
Cradle is a form-based tool for adding new items to Mare Nostrum Thesaurus dictionaries. Use it when you need to create a record in a specific dictionary (vessel part, provenance, etc.).
...
1. In the **Dictionaries** section, select a link in the form **New** and dictionary name (**New amphora type**, **New vessel part**, etc.).
...
1. Sign in with your Mare Nostrum Wiki account.
    ...
    ???+ note "Account required"
        If you are already logged in, you will not see login page.
...
1. **Labels** (required) –  name of the new item in the selected language
1. **Also known as** (optional) – alternative names (aliases) in the selected language
1. **Descriptions** (recommended) – short description in the selected language that distinguishes the item from similar ones
1. **Subclass of/Part of** (required) – parent item that places the new record in the dictionary hierarchy
1. **Additional properties** – dictionary-specific fields; whether they are required depends on the dictionary

Important interface elements:

- **Language selector** – a dropdown before each text field in **Labels**, **Also known as**, and **Descriptions**. Choose appropriate language for the label, description, or alias you are entering.
- **Add (+) and remove (−) buttons** – shown after each text field. Click `+` to add another entry in a different language (pick the language in the dropdown first); click `−` to remove an extra language entry.
...
![New chronology form showing described above elements and its dictionary-specific fields](cradle/newcolorpage.png)
...
By default, **subclass of/part of** points to the top-level parent of the dictionary. You can instead place the new item under another existing dictionary entry.

Example: add **example paphos** as a subtype of **paphos**, rather than directly under **amphora type**.

1. Open the Cradle form for the target dictionary.
1. In **subclass of**, replace the default parent with the existing item that should be the direct parent.
