...
???+ note "Dictionaries structure"
    Each dictionary is represented by a drop-down list that can be opened by clicking the corresponding link name or by clicking proper dictionary name in the [Available Data](#available-data) subsection.
...
1. To display a specific dictionary value, left-click (LMB) on the blue "Q" identifier followed by a corresponding number.
